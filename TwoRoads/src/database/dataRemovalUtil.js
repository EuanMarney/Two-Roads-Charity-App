...
export const dropAllTables = async () => {
    const tables = ['HedonicMoments', 'UserPreferences', 'ActsOfKindness', 'GratitudeDiary'];
  
    tables.forEach(table => {
      db.transaction(tx => {
        tx.executeSql(`DROP TABLE IF EXISTS ${table};`, [], (_, { rows }) => {
        },
        (t, error) => {
          console.log(`Error occurred while dropping the table ${table}: `, error);
